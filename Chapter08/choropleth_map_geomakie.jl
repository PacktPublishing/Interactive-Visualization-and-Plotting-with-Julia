...
download("https://datahub.io/core/geo-nuts-administrative-boundaries/r/nuts_rg_60m_2013_lvl_1.geojson", 
	"nuts_rg_60m_2013_lvl_1.geojson")

# ╔═╡ 0486d856-4849-43bc-80f4-294eba04f422
nuts = GeoJSON.read(read("nuts_rg_60m_2013_lvl_1.geojson"))

# ╔═╡ f108d637-147e-4188-bf85-f29246238a82
shape_areas = [feature.properties["SHAPE_AREA"] 
	for feature in nuts.features]

# ╔═╡ 219359cd-acc5-4874-800e-21c1214e3229
poly(nuts, color=shape_areas)
...
begin
	...
	ax = GeoAxis(fig[1,1], 
		coastlines = true, 
		coastline_attributes = (color=:lightgrey,),
		dest = "+proj=natearth")
	poly!(ax, nuts, color=shape_areas)
	cb = Colorbar(fig[1,2]; 
		colorrange = extrema(shape_areas), 
		label = "shape area",
